...
class no_ref_back_projection(nn.Module):
    def __init__(self, in_channels, stride):
        ...
        self.feat_fusion = nn.Sequential(nn.Conv2d(int(in_channels*1.5), in_channels, 3, stride=1, padding=1), nn.GELU())        
        self.feat_expand = nn.Sequential(nn.Conv2d(in_channels, int(in_channels*1.5), 3, stride=1, padding=1), nn.GELU())
        self.diff_fusion = nn.Sequential(nn.Conv2d(int(in_channels*1.5), in_channels, 3, stride=1, padding=1), nn.GELU())
        
        self.encoder1 = nn.Sequential(*[BFA(dim=int(in_channels*(1.5)), num_heads=1, stride=stride, ffn_expansion_factor=2.66, bias=bias, LayerNorm_type='WithBias') for i in range(2)])

    def burst_fusion(self, x):
        b, f, H, W = x.size()
        x = x.view(-1, int(f*1.5), H, W)
        return x
    ...
    def forward(self, x):
        ...
        feat = x.view(-1, int(f*1.5), H, W)
        shifted_feat = shifted_x.view(-1, int(f*1.5), H, W)
...
class no_ref_back_projection_ori(nn.Module):
    def __init__(self, in_channels, stride):

        super(no_ref_back_projection_ori, self).__init__()

        bias = False
        
        self.feat_fusion = nn.Sequential(nn.Conv2d(in_channels, in_channels, 3, stride=1, padding=1), nn.GELU())        
        self.feat_expand = nn.Sequential(nn.Conv2d(in_channels, in_channels, 3, stride=1, padding=1), nn.GELU())
        self.diff_fusion = nn.Sequential(nn.Conv2d(in_channels, in_channels, 3, stride=1, padding=1), nn.GELU())
    ...
    def forward(self, x):
        ...
        shifted_x = torch.roll(x, 1, 0)
        feat = x.view(-1, f, H, W)
        shifted_feat = shifted_x.view(-1, f, H, W)
        feat = self.encoder1(torch.cat([feat, shifted_feat], dim=0))
...
class Burstormer(nn.Module):
    def __init__(self, num_features=32, burst_size=8, reduction=8, bias=False):
        ...
        self.conv1 = nn.Sequential(
            nn.Conv2d(3, num_features, kernel_size=3, padding=1, bias=bias)
        )
        ## 改 channel 4->3
        # self.align = EDA(num_features)

        self.back_projection1 = no_ref_back_projection(num_features, stride=1)
        self.back_projection2 = no_ref_back_projection_ori(num_features, stride=1)
        
        
        self.rec_conv1 = nn.Sequential(nn.Conv2d(num_features*4, num_features, kernel_size=1, stride=1, padding=0, bias=False), nn.GELU())
...
if __name__ == "__main__":
    ...
    x = torch.randn(1, 3, 3 , 512, 512) #  flops:161.379155968,params:20.446435
